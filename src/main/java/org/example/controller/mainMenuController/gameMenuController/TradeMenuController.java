...
public class TradeMenuController {
    ...
    public Outputs trade(String resourceType, int resourceAmount, int price, String message) {
        ...
        if (!getThisEmpire().checkCapacity(materialType, resourceAmount)) return Outputs.NOT_ENOUGH_CAPACITY;
        int id = tradeMenu.getEmpireForTrade().getTradeHistory().size() + tradeMenu.getEmpireForTrade().getTrades().size() + 1;
        Trade trade = new Trade(id, new Material(materialType), resourceAmount, price, message, getThisEmpire(), tradeMenu.getEmpireForTrade());
    ...
    public static Outputs trade(MaterialType materialType, int resourceAmount, int price, String message) {
        if (materialType == null) return Outputs.INVALID_MATERIAL_TYPE;
        if (resourceAmount <= 0) return Outputs.INVALID_RESOURCE_AMOUNT_TRADE;
        if (!getThisEmpire().checkCapacity(materialType, resourceAmount)) return Outputs.NOT_ENOUGH_CAPACITY;
        int id = TradeMenuApp.getEmpireForTrade().getTradeHistory().size() + TradeMenuApp.getEmpireForTrade().getTrades().size() + 1;
        Trade trade = new Trade(id, new Material(materialType), resourceAmount, price, message, getThisEmpire(), TradeMenuApp.getEmpireForTrade());
        TradeMenuApp.getEmpireForTrade().addToTrades(trade);
        TradeMenuApp.getEmpireForTrade().addToNewTrade(trade);
        getThisEmpire().addToTradeHistory(trade);
        return Outputs.SUCCESS;
    }
    ...
    public Outputs tradeAccept(int id, String message) {
        ...
        if (trade.getEmpireRequester().getGold() < trade.getPrice()) return Outputs.TRADE_NOT_ENOUGH_GOLD;
    ...
    public static Outputs staticTradeAccept(int id, String message) {
        Trade trade = getThisEmpire().getTradeWhitId(id);
        if (trade == null) return Outputs.INVALID_ID;
        if (!getThisEmpire().havingMaterial(trade.getMaterial().getMaterialType(), trade.getAmountMaterial()))
            return Outputs.NOT_ENOUGH_MATERIAL;
